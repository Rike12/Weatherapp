...
var time = document.querySelector("#dayAndTime");
time.innerHTML = "".concat(day, " ").concat(hour, ":").concat(minute);
function checkWeather(response) {
  ...
  temperatureElement.innerHTML = "".concat(temperature, "\xB0C");
  var precipitationElement = document.querySelector("#precipitation");
  var precipitation = Math.round(response.data.clouds.all);
  precipitationElement.innerHTML = "Precipitation: ".concat(precipitation, "%");
  var windElement = document.querySelector("#wind");
  var windSpeed = Math.round(response.data.wind.speed);
  windElement.innerHTML = "Wind Speed: ".concat(windSpeed, "km/h");
  var humidityElement = document.querySelector("#humidity");
  var humidity = Math.round(response.data.main.humidity);
  humidityElement.innerHTML = "Humidity: ".concat(humidity, "%");
}
...
var submitForm = document.querySelector("#form-container");
submitForm.addEventListener("submit", cityInput);

// Bonus Task
function showCurrentWeather(response) {
  var currentPosition = document.querySelector("#city");
  console.log(response);
  var position = response.data.name;
  currentPosition.innerHTML = "".concat(position);
  var currentTemperature = document.querySelector("#temperature");
  var temperature = Math.round(response.data.main.temp);
  currentTemperature.innerHTML = "".concat(temperature, "\xB0C");
  var weatherCondition = document.querySelector("#weather-condition");
  var condition = response.data.weather[0].main;
  weatherCondition.innerHTML = "".concat(condition);
  var precipitationElement = document.querySelector("#precipitation");
  var precipitation = Math.round(response.data.clouds.all);
  precipitationElement.innerHTML = "Precipitation: ".concat(precipitation, "%");
  var windElement = document.querySelector("#wind");
